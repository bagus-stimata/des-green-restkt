...
import com.erp.distribution.desgreenrestkt.security_model.FUserVendors
import com.erp.distribution.desgreenrestkt.security_model.Role
import com.erp.distribution.desgreenrestkt.security_model.Role.allRoles
import com.erp.distribution.desgreenrestkt.security_repository.FUserRolesJPARepository
import com.erp.distribution.desgreenrestkt.security_repository.FUserVendorsJPARepository
...
class UsersController {
    ...
    fun init() {
        ...
        if (activeCompany != null) {
            ...
            listFSalesman = fSalesmanJPARepository!!.findAllByCompany(activeCompany!!.id)
                    .stream().filter { x: FSalesman -> x.isStatusActive == true && x.spcode != "" && x.spname != "" && (x.salesType == EnumSalesType.TO || x.salesType == EnumSalesType.C || x.salesType == EnumSalesType.C) }.collect(Collectors.toList())
            listFVendor = fVendorJPARepository!!.findAllByCompany(activeCompany!!.id).stream().filter { x: FVendor -> x.isStatusActive == true && x.vcode != "" && x.vname != "" }.collect(Collectors.toList())
        } else {
    ...
    var activeCompany: FCompany? = null
