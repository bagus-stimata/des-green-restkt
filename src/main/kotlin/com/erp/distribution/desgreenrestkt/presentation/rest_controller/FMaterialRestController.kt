...
//    @Autowired
//    lateinit var getFMaterialUseCase: GetFMaterialUseCase

    var fMaterialRepo: FMaterialRepo = FMaterialRepoImpl()
    ...
    fun getFMaterialById(@PathVariable("id") id: Int): FMaterialEntity {
    ...
    val allMaterialEntity: List<FMaterialEntity>
    ...
    fun getAllMaterialWithPageable(@RequestParam("page") page: Int, @RequestParam("size") size: Int): List<FMaterialEntity> {
        val pageable: Pageable = PageRequest.of(page, size)
        return fMaterialJPARepository.findAll(pageable).content.stream().filter { x: FMaterialEntity -> x.statusActive == true && x.pcode != "" && x.pname != "" }.collect(Collectors.toList())
    }

    @RequestMapping(value = ["/rest/getAllFMaterialByDivision/{fdivisionBean}"], produces = [MediaType.APPLICATION_JSON_VALUE])
    fun getAllFMaterialByDivision(@PathVariable("fdivisionBean") fdivisionBean: Int): List<FMaterialEntity> {
        activeUser = fUsersJPARepository.findById(securityUtils.loginUser!!.id).get()
...
//            fMaterialJPARepository.findAllByDivisionAndListVendor(fdivisionBean, listVendorsBean).stream().filter { x: FMaterialEntity -> x.statusActive == true && x.pcode != "" && x.pname != "" }.collect(Collectors.toList())
            fMaterialJPARepository.findAllByDivisionAndListVendor(fdivisionBean, listVendorsBean).stream().filter { x: FMaterialEntity -> x.statusActive == true && x.pcode != "" && x.pname != "" }.collect(Collectors.toList())
        } else {
//            fMaterialJPARepository.findAllByDivision(fdivisionBean).stream().filter { x: FMaterialEntity -> x.statusActive == true && x.pcode != "" && x.pname != "" }.collect(Collectors.toList())
            fMaterialJPARepository.findAllByDivision(fdivisionBean).stream().filter { x: FMaterialEntity -> x.statusActive == true && x.pcode != "" && x.pname != "" }.collect(Collectors.toList())
        }
    }
    ...
    fun getAllFMaterialByDivisionAndShareToCompany(@PathVariable("fdivisionBean") fdivisionBean: Int, @PathVariable("fcompanyBean") fcompanyBean: Int): List<FMaterialEntity> {
...
//        System.out.println("aktif user: " + activeUser.getUsername() + " >> " + activeUser.getfUserVendors());
        return if (activeUser.fUserVendors.size > 0) {
            ...
            fMaterialJPARepository.findAllByDivisionAndShareToCompanyAndListVendor(fdivisionBean, fcompanyBean, listVendorsBean).stream().filter { x: FMaterialEntity -> x.statusActive == true && x.pcode != "" && x.pname != "" }.collect(Collectors.toList())
        } else {
            fMaterialJPARepository.findAllByDivisionAndShareToCompany(fdivisionBean, fcompanyBean).stream().filter { x: FMaterialEntity -> x.statusActive == true && x.pcode != "" && x.pname != "" }.collect(Collectors.toList())
        }
    }

    @RequestMapping(value = ["/rest/getAllFMaterialByDivisionPage/{fdivisionBean}"], produces = [MediaType.APPLICATION_JSON_VALUE])
    fun getAllFMaterialByDivision(@PathVariable("fdivisionBean") fdivisionBean: Int, @RequestParam("page") page: Int, @RequestParam("size") size: Int): List<FMaterialEntity> {
        val pageable: Pageable = PageRequest.of(page, size)
